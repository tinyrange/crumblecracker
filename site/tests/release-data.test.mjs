...
for (const includeShell of [false, true]) {
  test(`release refresh accepts desktop releases${includeShell ? " and ignores legacy shell assets" : " without a shell asset"}`, async (t) => {
    ...
    const latest = {
      ...
      assets: products.flatMap((product) => ["amd64", "arm64"].map((arch) => ({
        name: `${product}_v1.0.0_linux_${arch}`,
        size: 1024,
        browser_download_url: `https://github.com/tinyrange/crumblecracker/releases/download/v1.0.0/${product}_v1.0.0_linux_${arch}`,
      }))),
    };
    ...
    assert.deepEqual(actual.assets.map((asset) => asset.product), ["NeurodeskAppX", "NeurodeskAppX", "SquadVM", "SquadVM"]);
    for (const product of ["NeurodeskAppX", "SquadVM"]) {
      const assets = actual.assets.filter((asset) => asset.product === product);
      assert.deepEqual(assets.map((asset) => asset.arch), ["x64", "ARM64"]);
      assert.ok(assets[0].url.endsWith("linux_amd64"));
      assert.ok(assets[1].url.endsWith("linux_arm64"));
    }
